...
class TreePlot:
      ...
      def DrawTrees(self,filename,treename,drawname,index,xtitle,ytitle,scalename,lumi,keyname,dict,multiplicitylabel,plotdir):
          ...
          if os.path.isfile(filename):
            ...
            elif ((len(drawname.split("-")) > 1) or (len(drawname.split("+")) > 1) or (len(drawname.split("*")) > 1) or (len(drawname.split("/")) > 1)):
               #print "Type2"
               obj.Draw("%s>>h%d(%d,%f,%f)"%(drawname,self.n,self.bin_,self.min_,self.max_),"((%s))"%(self.cutname_),"goff")
            else: 
               print "Type3"
               index = float(index)
