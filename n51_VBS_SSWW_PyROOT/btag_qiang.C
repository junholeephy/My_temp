...
void AnalyseEvents(ExRootTreeReader *treeReader, TString filename)
{
  ...
  double ptVBFJet[99],etaVBFJet[99],phiVBFJet[99],massVBFJet[99];
  //double ptvbfj1,etavbfj1,phivbfj1,massvbfj1,ptvbfj2,etavbfj2,phivbfj2,massvbfj2;
  //double ptvbfjj,etavbfjj,phivbfjj,massvbfjj;
  ...
  tree->Branch("numvbfJet", &numvbfJet, "numvbfJet/I");
  ...
  tree->Branch("ptVBFJet", &ptVBFJet, "ptVBFJet[numvbfJet]/D");
  tree->Branch("etaVBFJet", &etaVBFJet, "etaVBFJet[numvbfJet]/D");
  tree->Branch("phiVBFJet", &phiVBFJet, "phiVBFJet[numvbfJet]/D");
  tree->Branch("massVBFJet", &massVBFJet, "massVBFJet[numvbfJet]/D");
  ...
  for(int count=0; count < numberOfEntries; count++)
  {
      ...
      for(int i=0; i<99; i++)
      {
          ptVBFJet[i]=-99;etaVBFJet[i]=-99;phiVBFJet[i]=-99;massVBFJet[i]=-99;
      ...
      for(int i=0, j=0, k=0; i<numJet; i++)
      {
	      ...
	      pJet[i]=(Jet*)branchJet->At(i);
              ...
              if( (!(btagJet[i] & (1 << 0))) && pJet[i]->PT>25 && fabs(pJet[i]->Eta)<4.7 )
	      {
		  ptVBFJet[j]=pJet[i]->PT;
                  etaVBFJet[j]=pJet[i]->Eta;
                  phiVBFJet[j]=pJet[i]->Phi;
                  massVBFJet[j]=pJet[i]->Mass;
		  numvbfJet++;j++;
	      }
      ...
      //if( (!(numvbfJet>=2&&numbJet>=2)) && (!(numvbfJet>=3&&numbJet>=1)) )continue;
      //if( (numvbfJet<2||numbJet<2) && (numvbfJet<3||numbJet<1) )continue;
      if(numvbfJet<2||numbJet<1)continue;
...
void ntuple_chain()
{
  gSystem->Load("/home/qliphy/Desktop/Snowmass/MG5_aMC_v2_2_3/Delphes/libDelphes");
  //THStack *stack
  // access to 0-600 HT bin files
  TChain *chain = new TChain("Delphes");
  chain->Add("/home/qliphy/Desktop/Snowmass/Data/others/sig0p1.root");
