...
void n4_VBSWW_madspin_decayed_Entry_Normalized_ptl1()
{
    ...
    TH1F *hLL = new TH1F("hLL","VBS SS WW, Normalized with Entry number ",20,20,400);  hLL->SetLineColor(kRed);
    TH1F *hTT = new TH1F("hTT","hTT",20,20,400);  hTT->SetLineColor(kGreen);
    TH1F *hTL = new TH1F("hTL","hTL",20,20,400);  hTL->SetLineColor(kBlack);
    
    TH1F *htest = new TH1F("htest","htest",20,20,400); htest->SetLineColor(kMagenta); //htest->SetLineStyle(10);
...
//    hLLTTTL->Scale(scale_lltttl);    hLLTTTL->Draw("hist");
    hLL->Scale(scale_ll); hLL->GetXaxis()->SetTitle("pt_lep1");   hLL->Draw("hist");
